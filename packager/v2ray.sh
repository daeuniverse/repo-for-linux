...
for rpm_arch in x86_64 aarch64 i386 riscv64; do
    cat nfpm/v2ray.yaml | sed "s/REAL_VERSION/$REAL_VERSION/g" | sed "s/REAL_ARCH/${rpm_arch}/g" | tee /tmp/v2ray_${REAL_VERSION}_${rpm_arch}_rpm.yaml
    nfpm package -p rpm --config /tmp/v2ray_${REAL_VERSION}_${rpm_arch}_rpm.yaml --target ./archive/v2ray_${REAL_VERSION}_${rpm_arch}.rpm
done