...
for debian_arch in amd64 arm64 i386 riscv64; do
    ...
    nfpm package -p deb --config /tmp/v2ray_${REAL_VERSION}_${debian_arch}_.yaml --target ./archive/deb/v2ray_${REAL_VERSION}_${debian_arch}.deb
    nfpm package -p rpm --config /tmp/v2ray_${REAL_VERSION}_${debian_arch}_.yaml --target ./archive/rpm/v2ray_${REAL_VERSION}_${debian_arch}.rpm
done