...
for debian_arch in amd64 arm64 i386 riscv64; do
    ...
    nfpm package -p deb --config /tmp/juicity_${REAL_VERSION}_${debian_arch}_.yaml --target ./archive/deb/juicity_${REAL_VERSION}_${debian_arch}.deb
    nfpm package -p rpm --config /tmp/juicity_${REAL_VERSION}_${debian_arch}_.yaml --target ./archive/rpm/juicity_${REAL_VERSION}_${debian_arch}.rpm
done