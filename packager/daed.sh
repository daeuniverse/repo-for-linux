...
for rpm_arch in x86_64 aarch64 i386 riscv64; do
    cat nfpm/daed.yaml | sed "s/REAL_VERSION/$REAL_VERSION/g" | sed "s/REAL_ARCH/${rpm_arch}/g" | tee /tmp/daed_${REAL_VERSION}_${rpm_arch}_rpm.yaml
    nfpm package -p rpm --config /tmp/daed_${REAL_VERSION}_${rpm_arch}_rpm.yaml --target ./archive/daed_${REAL_VERSION}_${rpm_arch}.rpm
done