...
- 命中 `must_direct` 的流量。區域網路用戶端發往 dae 主機自身 socket 的查詢（例如本機監聽連接埠 53 的 dnsmasq）也和其它報文一樣走路由，UDP 與 TCP 都會進入 DNS 模組；要讓本機解析器直接應答，用 `l4proto(udp) && dport(53) && dip(<dae 主機位址>) -> must_direct` 顯式表達。
...
因為 dae 會接管所有經它路由的 DNS 查詢，所以區域網路裝置的 DNS 伺服器填任意位址即可，填 dae 主機自身也可以。使用 `asis` 時，不要讓區域網路裝置把 dae 自身的連接埠 53 當作 DNS 伺服器，否則查詢會在 dae 與自身之間形成環路。設定 `dns.bind`（例如 `'127.0.0.1:5353'`）後，dae 還會在該位址監聽 DNS 查詢。
